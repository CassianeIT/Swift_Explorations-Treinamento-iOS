...
func responseTo1(question: String) -> String {
  
let convertLetter = question.lowercased()
    print(convertLetter)
    
    if convertLetter == "Where are the cookies?" {
        return "In the cookie jar!"
    } else if convertLetter.hasPrefix("where") {
        return "In the cookie jar!"
    } else {
        return "That really depends"
    }
